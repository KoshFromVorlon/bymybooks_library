...
# Имя главной папки
base_folder = "My_Library"


# Функция для очистки имени папки от запрещенных в Windows символов
def sanitize_folder_name(name):
    # Запрещенные символы в Windows: < > : " / \ | ? *
    # Меняем двоеточие на тире, остальное на подчеркивание
    invalid_chars = '<>"|?*'

    # Сначала заменим слэши и двоеточия на что-то более красивое
    name = name.replace(':', ' -')
    name = name.replace('/', '-')
    name = name.replace('\\', '-')

    # Остальные запрещенные символы меняем на _
...
if not os.path.exists(base_folder):
    ...
    print(f"Папка '{base_folder}' создана.")
else:
    print(f"Папка '{base_folder}' уже существует.")

# Разбиваем текст на строки и создаем папки
count = 0
for line in books_text.strip().split('\n'):
    clean_line = line.strip()
    if not clean_line:
        continue

    # Чистим имя
    folder_name = sanitize_folder_name(clean_line)

    # Полный путь
    full_path = os.path.join(base_folder, folder_name)

    try:
        # Создаем папку (exist_ok=True не выдаст ошибку, если папка уже есть)
        os.makedirs(full_path, exist_ok=True)
        count += 1
    except Exception as e:
        print(f"Ошибка при создании папки '{folder_name}': {e}")

print(f"\nГотово! Создано (или проверено) папок: {count}")
print(f"Ищите их в папке: {os.path.abspath(base_folder)}")